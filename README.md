# JOOQ Codegen Convention Plugin

A framework-agnostic Gradle convention plugin for generating JOOQ classes from SQL migrations (PostgreSQL) for Kotlin-based projects.

This plugin simplifies the JOOQ code generation workflow by leveraging **Testcontainers**. It automatically spins up a PostgreSQL container, applies your SQL migrations to it, generates the JOOQ Java/Kotlin classes against that actual schema, and then cleans up.

## Features

- **Automated Database Lifecycle**: Starts a PostgreSQL Testcontainer before generation and shuts it down afterward.
- **Migration-based Generation**: Uses your actual SQL migrations (`src/main/resources/db/migration`) to define the schema.
...
- **Dependency Management**: Automatically applies necessary dependencies (JOOQ, PostgreSQL test container etc).
...
```kotlin
plugins {
    id("io.github.usmanzaheer1995.jooq-codegen-convention-plugin") version "2.0.1"
}
```
...
jooqConventions { 
    ...
    // required parameter - resolves relative to the consumer project's root
...
The plugin expects your SQL migration scripts to be located in the standard path:
```text
...
1. **`setupDatabaseForJooq`**: Starts a PostgreSQL Docker container and runs the SQL migrations found in `src/main/resources/db/migration`.
